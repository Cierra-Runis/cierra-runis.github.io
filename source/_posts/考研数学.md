...
例力场 $\vec{F}(x, y) = \begin{bmatrix}F_x(x, y) \\ F_y(x, y)\end{bmatrix}$，则做功 $W_{\vec{L}} = \displaystyle\int_{\vec{L}} \vec{F}\mathrm{d}\vec{s} = \displaystyle\int_{\vec{L}} F_x\mathrm{d}x + F_y\mathrm{d}y$
...
利用 $W_{\vec{L}} = -W_{-\vec{L}}$，我们可以将环分割为无穷多小区域 $\mathrm{d}\vec{S}$
...
要提醒的一点是 $\vec{\nabla} \times \vec{F}$ 不完全是叉乘，而是表示求旋度的意思，即 $\mathrm{rot} \vec{F}$
...
例面密度 $\rho(x, y, z)$，则面质量 $M = \displaystyle\iint_{\Sigma} \rho(x, y, z) \mathrm{d}S$

一般来说，该面 $\Sigma$ 会以 $z = z(x, y)$ 的形式给出，从而有 $\mathrm{d}S = \sqrt{1 + (\dfrac{\mathrm{d}z}{\mathrm{d}x})^2 + (\dfrac{\mathrm{d}z}{\mathrm{d}y})^2} \mathrm{d}x\mathrm{d}y$
...
同理 $y$ 方向有 $\vec{v_2} = (1, 0, \dfrac{\partial{z}}{\partial{y}}) \mathrm{d}y$
...
$$
M = \iint_{\Sigma} F(x,y) z\mathrm{d}S = \iint_{D} F(x, y) \sqrt{z^2 + (z\dfrac{\mathrm{d}z}{\mathrm{d}x})^2 + (z\dfrac{\mathrm{d}z}{\mathrm{d}y})^2} \mathrm{d}x\mathrm{d}y
$$
...
被积曲面为 $\Sigma(x, y, z)$，令 $x = x(u, v)$，$y = y(u, v)$，$z = z(u, v)$——你可能会想这怎么可能，其实代换到生活中经纬度和地球上某点的映射就理解了

其在 $u$ 方向上有极小变动 $\mathrm{d}u$，则变动向量 $\vec{v_1} = (\dfrac{\partial{x}}{\partial{u}}, \dfrac{\partial{y}}{\partial{u}}, \dfrac{\partial{z}}{\partial{u}}) \mathrm{d}u = \dfrac{\partial{\vec{\Sigma}}}{\partial{u}} \mathrm{d}u$

同理 $v$ 方向有 $\vec{v_2} = \dfrac{\partial{\vec{\Sigma}}}{\partial{v}} \mathrm{d}v$

可得面积微元即两个向量的叉积的大小，即：

$$
\mathrm{d}S = |\vec{v_1} \times \vec{v_2}| \xlongequal{一系列推导} \sqrt{(\dfrac{\partial{\vec{\Sigma}}}{\partial{u}})^2 (\dfrac{\partial{\vec{\Sigma}}}{\partial{v}})^2 - (\dfrac{\partial{\vec{\Sigma}}}{\partial{u}} \dfrac{\partial{\vec{\Sigma}}}{\partial{v}})^2 } \mathrm{d}u\mathrm{d}v
$$
...
散度表示为 $\vec{\nabla} \cdot \vec{F}$，这是完全的点乘操作，其结果是个数值
