...
| :--------: | :---------------------------------: | :----------------------------------------------------: | :--------------------------------------: | :-------------: | :-----------------------------------------------------: |
|  二项分布  |          $X \sim B(n, p)$           |             记 $X$ 为 $n$ 次实验中出现次数             |           $C_n^k p^k (1-p)^k$            |      $np$       |                       $np(1 - p)$                       |
|  0-1 分布  |          $X \sim B(1, p)$           |                           略                           |                    略                    |       略        |                           略                            |
| 超几何分布 |         $X \sim H(n, M, N)$         | 记 $X$ 为含 $M$ 特殊的总体 $N$ 中取 $n$ 次所取特殊体数 | $\dfrac{C_M^k C_{N - M}^{n - k}}{C_N^n}$ | $\dfrac{nM}{N}$ | $\dfrac{nM}{N} (1 - \dfrac{M}{N}) \dfrac{N - n}{N - 1}$ |
| 负二项分布 |          $X \sim Nb(r, p)$          |         记 $X$ 为事件第 $r$ 次出现时的实验次数         |  $C_{k - 1}^{r - 1} p^r (1-p)^{k - r}$   | $\dfrac{r}{p}$  |                 $\dfrac{r(1 - p)}{p^2}$                 |
|  几何分布  | $X \sim Nb(1, p)$ 或 $X \sim Ge(p)$ |                           略                           |                    略                    |       略        |                           略                            |
|  泊松分布  |         $X \sim P(\lambda)$         |                           略                           |   $\dfrac{\lambda^k}{k!}e^{-\lambda}$    |    $\lambda$    |                        $\lambda$                        |

### 常见连续型随机变量的概率分布、期望与方差

|   名称   |           记法            | 意义 |                           概率密度 $f_X(x)$                            |         期望         |          方差           |
| :------: | :-----------------------: | :--: | :--------------------------------------------------------------------: | :------------------: | :---------------------: |
| 均匀分布 |     $X \sim U[a, b]$      |  略  |                    $\dfrac{1}{b - a}, x \in [b, a]$                    |  $\dfrac{a + b}{2}$  | $\dfrac{(b - a)^2}{12}$ |
| 正态分布 | $X \sim N(\mu, \sigma^2)$ |  略  | $\dfrac{1}{\sqrt{2\pi\sigma^2}} \exp{-\dfrac{(x - \mu)^2}{2\sigma^2}}$ |        $\mu$         |       $\sigma^2$        |
| 指数分布 |   $X \sim Exp(\lambda)$   |  略  |                $\lambda e^{-\lambda x}, x \geqslant 0$                 | $\dfrac{1}{\lambda}$ | $\dfrac{1}{\lambda^2}$  |
