...
##### 结合立方和公式将高阶求导转为等比数列求和

$f(x) = \dfrac{1}{4x^2 + 2x + 1}$，求 $f^{(100)}(0)$

由立方和公式 $a^3 - b^3 = (a - b)(a^2 + ab + b^2)$

得 $f(x) = \dfrac{1}{\dfrac{1^3 - (2x)^3}{1 - 2x}} = \dfrac{1}{1 - 8x^3} - \dfrac{2x}{1 - 8x^3}$，下略
...
$$
\mathrm{d}u = \dfrac{\partial{z}}{\partial{x}} \mathrm{d}x + \dfrac{\partial{z}}{\partial{y}} \mathrm{d}y
$$
...
- 对实数列 $\{a_n\}, \{b_n\}$ 且 $\{b_n\}$ 严格单调递增，并有 $\displaystyle\lim_{n \to +\infty} b_n = \pm \infty$
...
- $\displaystyle\sum_{k = 1}^{\infty} u_k$ 收敛 $\impliedby \displaystyle\lim_{k \to \infty} u_k = \omicron(\dfrac{1}{k})$
- 级数去掉有限项不影响级数敛散性
...
1. $\displaystyle\sum_{k = 1}^{\infty} v_k$ 绝对收敛且对所有足够大的 $k$ 有 $|u_k| \le |v_k|$，则 $\displaystyle\sum_{k = 1}^{\infty} u_k$ 也绝对收敛
2. $\displaystyle\sum_{k = 1}^{\infty} v_k$ 不绝对收敛且对所有足够大的 $k$ 有 $|u_k| \ge |v_k|$，则 $\displaystyle\sum_{k = 1}^{\infty} u_k$ 也不绝对收敛

注意：第二种情况 $\displaystyle\sum_{k = 1}^{\infty} u_k$ 可能条件收敛

##### 极限比较判别法

对 $\displaystyle\sum_{k = 1}^{\infty} u_k$ 和 $\displaystyle\sum_{k = 1}^{\infty} v_k$ 且对 $\forall{k \in \N^*}$ 有 $u_k, v_k \ge 0$，记 $I = \displaystyle\lim_{k \to \infty} \dfrac{a_n}{b_n}$
...
>
> 故 $A$ 的行秩 $\le R$ 的行秩 $\le r = A$ 的列秩
>
> 再考虑 $A^T$，则有 $A$ 的列秩 $= A^T$ 的行秩 $\le A^T$ 的列秩 $= A$ 的行秩
>
> 综上故两者相等

- 矩阵 $A$ 与 $B$ 等价 $\iff$ 矩阵 $A$ 与 $B$ 同形且 $r(A) = r(B)$

> 原本的定义 $B_{m \times n} = Q_{m \times m}A_{m \times n}P_{n \times n}$ 且 $P, Q$ 均可逆其实就暗含了这点
>
> 两者对应了同一向量空间

- $0 \le r(A_{m \times n}) < \min{\{m, n\}}$
